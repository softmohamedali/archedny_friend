...
class MessagesAdapters (): RecyclerView.Adapter<MessagesAdapters.VH>(){
    ...
    private var messages= listOf<TextMassage>()
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        if (currentMassage.senderId==currentUserId){
            holder.view.tvMessageContent.setBackgroundResource(R.drawable.chat_send_bubble_arrow)
            holder.view.textMsgContainer.gravity=Gravity.END
        }else{
    ...
    fun setData(newFriends: List<TextMassage>) {
        mylog(newFriends.toString(),this)
        messages = newFriends
